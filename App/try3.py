...
# Dropdown settings
dropdown_width = 250  # Dropdown width
dropdown_height = 50
dropdown_open = False

# Fonts
font = pygame.font.Font(None, 25)  # Default font
retropix_font = pygame.font.Font(os.path.join(FONT_DIR, "retropix.ttf"), 100)
...
while running:
    ...
    # Draw UI Elements
    label_text = font.render("Select Device:", True, (255, 255, 255))
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Scan Button Clicked
            if (mouse_x - scan_pos[0] - BUTTON_RADIUS) ** 2 + (mouse_y - scan_pos[1] - BUTTON_RADIUS) ** 2 <= BUTTON_RADIUS ** 2:
                available_ports = get_serial_ports()
                selected_device = available_ports[0] if available_ports else "No device connected"

            # Connect Button Clicked
            elif (mouse_x - connect_pos[0] - BUTTON_RADIUS) ** 2 + (mouse_y - connect_pos[1] - BUTTON_RADIUS) ** 2 <= BUTTON_RADIUS ** 2:
